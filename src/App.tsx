...
function App() {
      ...
      for (let i = 0; i < n; i++) {
        switch (i % 3) {
          case 0:
            ...
            break;
          case 1:
            fill = 'rgba(0, 240, 40, 0.62)'
            break;
          case 2:
            fill = 'rgba(0, 240, 240, 0.82)'
            break;
        }
        let b = generateAndValidateBox(bs, fill);
        if (b) bs.push(b);
      }
      setBoxes(bs)
    }
  ...
  function generateAndValidateBox(bs: BoxShape[], fill: any): BoxShape | null {
  ...
  return (
    <div style={{ display: 'flex', }}>
      <div style={{ flexDirection: 'row' }}>
        <div>
          <button onClick={draw}>Generate Lines</button>
          <button onClick={clearAll}>CLEAR</button>
        </div>

      </div>
      <CanvasArea>
        {[...hLines, ...vLines].map((line: Line) =>
          line.renderLine()
        )}
